...
if (!/^https:\/\//.test(config.publicBaseUrl)) throw new Error('PUBLIC_BASE_URL musi być publicznym adresem HTTPS dostępnym dla Vapi.');
if (!config.voice.webhookSecret) console.warn('UWAGA: VOICE_WEBHOOK_SECRET jest pusty. Skonfiguruj Vapi Custom Credential przed pilotem.');
if (config.voice.webhookSecret && !vapi.serverCredentialId) console.warn('UWAGA: dodaj VAPI_SERVER_CREDENTIAL_ID, aby Vapi wysyłało sekret do webhooka.');

const assistant = buildVapiAssistantConfig(config);
const endpoint = vapi.assistantId ? `${vapi.apiUrl}/assistant/${vapi.assistantId}` : `${vapi.apiUrl}/assistant`;
