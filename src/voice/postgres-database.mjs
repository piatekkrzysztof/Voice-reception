...
async function migrate(pool) {
  ...
  try {
    ...
    await client
      .query("SELECT pg_advisory_unlock(hashtext('voice-reception:migrations'))")
      .catch(() => {});
    client.release();
  }
}
...
export async function createPostgresDatabase({ config, tenantId, seedCalls = [] }) {
  ...
  for (const row of serviceRows) {
    await pool.query(
      `INSERT INTO voice_services (id, tenant_id, slug, name, duration_minutes)
  ...
  async function expireHolds(now = new Date().toISOString(), client = pool) {
    const result = await client.query(
      ...
      WHERE status IN ('active','confirming') AND expires_at <= $1`,
      [now],
    );
  ...
  async function listServices(currentTenant = tenantId) {
    const result = await pool.query(
      `SELECT id, slug, name, duration_minutes, external_event_type_id, active
      FROM voice_services WHERE tenant_id = $1 AND active = TRUE ORDER BY duration_minutes`,
      [currentTenant],
    );
  ...
  async function configureEventTypes(eventTypes, currentTenant = tenantId) {
    for (const [slug, eventTypeId] of Object.entries(eventTypes || {})) {
      if (eventTypeId)
        await pool.query(
          `UPDATE voice_services SET external_event_type_id = $1
        WHERE tenant_id = $2 AND slug = $3`,
          [eventTypeId, currentTenant, slug],
        );
    }
  }

  async function slotIsFree(
    { tenantId: currentTenant = tenantId, serviceId, startAt, endAt },
    client = pool,
  ) {
    ...
    const result = await client.query(
      ...
      [currentTenant, serviceId, startAt, endAt],
    );
  ...
  async function createHold({
    ...
    return transaction(pool, async (client) => {
      await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', [
        `${currentTenant}:${serviceId}:${startAt.slice(0, 10)}`,
      ]);
  ...
  async function getHold(holdId, currentTenant = tenantId, client = pool) {
    await expireHolds(new Date().toISOString(), client);
    const result = await client.query(
      'SELECT * FROM voice_holds WHERE id = $1 AND tenant_id = $2',
      [holdId, currentTenant],
    );
  ...
  async function claimHold(holdId, currentTenant = tenantId) {
    return transaction(pool, async (client) => {
      await expireHolds(new Date().toISOString(), client);
      const result = await client.query(
        `UPDATE voice_holds SET status = 'confirming'
  ...
  async function confirmHold({ hold, customerName, phone, email, providerUid, idempotencyKey }) {
    return transaction(pool, async (client) => {
      ...
      const current = await client.query(
        `SELECT * FROM voice_holds
        WHERE id = $1 AND tenant_id = $2 FOR UPDATE`,
        [hold.id, hold.tenantId],
      );
  ...
  async function listBookings(currentTenant = tenantId, limit = 50) {
    const result = await pool.query(
      `SELECT b.*, s.name AS service_name, s.duration_minutes
      FROM voice_bookings b JOIN voice_services s ON s.id = b.service_id AND s.tenant_id = b.tenant_id
      WHERE b.tenant_id = $1 ORDER BY b.start_at DESC LIMIT $2`,
      [currentTenant, limit],
    );
  ...
  async function cancelBooking(bookingId, currentTenant = tenantId) {
    const result = await pool.query(
      `UPDATE voice_bookings b SET status = 'cancelled', updated_at = NOW()
      FROM voice_services s WHERE b.service_id = s.id AND b.tenant_id = s.tenant_id AND b.id = $1 AND b.tenant_id = $2
      RETURNING b.*, s.name AS service_name, s.duration_minutes`,
      [bookingId, currentTenant],
    );
    return mapBooking(result.rows[0]);
  }
  ...
  async function upsertCall(call) {
    ...
    const result = await pool.query(
      ...
      [
        ...
        call.bookingId || null,
        Boolean(call.transferred),
        call.cost || 0,
        call.aiDisclosure !== false,
        call.summary || null,
  ...
  async function listCalls(currentTenant = tenantId, limit = 50) {
    const result = await pool.query(
      `SELECT id, external_id, tenant_id, provider, started_at, ended_at,
      duration_seconds, caller, intent, outcome, booking_id, transferred, cost, ai_disclosure, summary, ended_reason
      FROM voice_calls WHERE tenant_id = $1 ORDER BY started_at DESC LIMIT $2`,
      [currentTenant, limit],
    );
  ...
  async function recordEvent({
    ...
    await pool.query(
      `INSERT INTO voice_events (id, tenant_id, call_id, type, detail, created_at)
      VALUES ($1,$2,$3,$4,$5,$6)`,
      [event.id, event.tenantId, event.callId, event.type, event.detail || {}, event.createdAt],
    );
    return event;
  }
  ...
  async function stats(currentTenant = tenantId) {
    const callsResult = await pool.query(
      ...
      FROM voice_calls WHERE tenant_id = $1`,
      [currentTenant],
    );
    const bookingsResult = await pool.query(
      `SELECT COUNT(*) AS bookings FROM voice_bookings
    ...
    return {
      ...
      bookings: bookingsResult.rows[0].bookings,
    };
  }
  ...
  async function createFirstAdmin({ email, passwordHash }) {
    return transaction(pool, async (client) => {
      ...
      await client.query(
        ...
        VALUES ($1,$2,$3,$4,$5,$5)`,
        [admin.id, admin.email, passwordHash, admin.role, admin.createdAt],
      );
      return admin;
    });
  }
  ...
  async function findAdminByEmail(email) {
    const result = await pool.query(
      ...
      FROM voice_admins WHERE LOWER(email) = LOWER($1)`,
      [email],
    );
  ...
  async function findAdminSession(tokenHash) {
    ...
    const result = await pool.query(
      `UPDATE voice_sessions s SET last_seen_at = NOW()
      FROM voice_admins a WHERE s.admin_id = a.id AND s.token_hash = $1 AND s.expires_at > NOW()
      RETURNING s.token_hash, s.expires_at, a.id AS admin_id, a.email, a.role`,
      [tokenHash],
    );
    const row = result.rows[0];
    return row
  ...
  async function deleteAdminSession(tokenHash) {
    const result = await pool.query('DELETE FROM voice_sessions WHERE token_hash = $1', [
      tokenHash,
    ]);
    return result.rowCount;
  }
