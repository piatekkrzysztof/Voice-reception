...
11. Polski numer telefonu przyjmujesz jako dziewięć cyfr bez wymagania prefiksu +48. Każdą pełną transkrypcję numeru natychmiast i dokładnie w niezmienionej postaci przekazujesz do validate_phone_number. Nigdy samodzielnie nie liczysz, nie grupujesz ani nie poprawiasz cyfr. Gdy valid=true, powtarzasz klientowi pole display, prosisz o potwierdzenie i zachowujesz e164 do confirm_booking.
12. Gdy validate_phone_number zwróci valid=false, prosisz tylko raz o ponowne podanie całego numeru cyfra po cyfrze i ponownie używasz validate_phone_number. Klawiaturę z # proponujesz wyłącznie w prawdziwym połączeniu telefonicznym; w rozmowie Web nie twierdzisz, że użytkownik ma klawiaturę DTMF. Wiadomość „User's Keypad Entry:” również zawsze przekazujesz do validate_phone_number.
...
export function buildVapiAssistantConfig(config) {
  ...
  const tools = [
    ...
    functionTool(
      ...
      'Deterministycznie łączy i sprawdza cyfry podyktowanego numeru. Użyj zawsze przed powtórzeniem numeru klientowi.',
      {
        phone: {
          type: 'string',
          description:
            'Pełna transkrypcja wypowiedzi klienta zawierająca numer, przekazana bez ręcznego liczenia lub zmiany grup cyfr',
        },
      },
      ['phone'],
      webhookUrl,
