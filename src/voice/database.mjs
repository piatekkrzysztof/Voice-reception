...
export function createVoiceDatabase({ path, tenantId, seedCalls = [] }) {
  ...
  const callCount = db.prepare('SELECT COUNT(*) AS count FROM voice_calls WHERE tenant_id = ?').get(tenantId).count;
  if (callCount === 0 && seedCalls.length) {
    ...
    for (const call of seedCalls) {
      const endedAt = new Date(new Date(call.startedAt).getTime() + call.durationSeconds * 1000).toISOString();
      insertCall.run(call.id, call.id, tenantId, 'seed', call.startedAt, endedAt, call.durationSeconds, call.caller, call.intent, call.outcome, call.booking || null, call.transferred ? 1 : 0, call.cost || 0, call.aiDisclosure === false ? 0 : 1, call.booking || null, 'seeded-demo-call', call.startedAt, endedAt);
    }
  }
  ...
  function expireHolds(now = new Date().toISOString()) {
    return db.prepare(`UPDATE voice_holds SET status = 'expired'
      WHERE status IN ('active','confirming') AND expires_at <= ?`).run(now).changes;
  }

  function listServices(currentTenant = tenantId) {
    return db.prepare(`SELECT id, slug, name, duration_minutes AS durationMinutes,
      external_event_type_id AS externalEventTypeId, active
      FROM voice_services WHERE tenant_id = ? AND active = 1 ORDER BY duration_minutes`).all(currentTenant)
      .map((row) => ({ ...row, active: Boolean(row.active) }));
  }

  function configureEventTypes(eventTypes, currentTenant = tenantId) {
    const update = db.prepare('UPDATE voice_services SET external_event_type_id = ? WHERE tenant_id = ? AND slug = ?');
  ...
  function slotIsFree({ tenantId: currentTenant = tenantId, serviceId, startAt, endAt }) {
    expireHolds();
    const booking = db.prepare(`SELECT id FROM voice_bookings
      WHERE tenant_id = ? AND service_id = ? AND status = 'confirmed'
      AND start_at < ? AND end_at > ? LIMIT 1`).get(currentTenant, serviceId, endAt, startAt);
    if (booking) return false;
    const hold = db.prepare(`SELECT id FROM voice_holds
      WHERE tenant_id = ? AND service_id = ? AND status IN ('active','confirming') AND expires_at > ?
      AND start_at < ? AND end_at > ? LIMIT 1`).get(currentTenant, serviceId, new Date().toISOString(), endAt, startAt);
    return !hold;
  }

  function createHold({ tenantId: currentTenant = tenantId, serviceId, startAt, endAt, expiresAt, providerReservationUid = null }) {
    return transaction(() => {
      expireHolds();
      ...
      db.prepare(`INSERT INTO voice_holds
        (id, tenant_id, service_id, start_at, end_at, status, provider_reservation_uid, expires_at, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(hold.id, hold.tenantId, hold.serviceId, hold.startAt, hold.endAt, hold.status, hold.providerReservationUid, hold.expiresAt, hold.createdAt);
      return hold;
    });
  }
  ...
  function getHold(holdId, currentTenant = tenantId) {
    expireHolds();
    const row = db.prepare(`SELECT id, tenant_id AS tenantId, service_id AS serviceId, start_at AS startAt,
      end_at AS endAt, status, provider_reservation_uid AS providerReservationUid,
      expires_at AS expiresAt, created_at AS createdAt FROM voice_holds WHERE id = ? AND tenant_id = ?`).get(holdId, currentTenant);
    return row || null;
  }
  ...
  function claimHold(holdId, currentTenant = tenantId) {
    return transaction(() => {
      ...
      db.prepare(`UPDATE voice_holds SET status = 'confirming' WHERE id = ? AND status = 'active'`).run(holdId);
      return { ...hold, status: 'confirming' };
    });
  }

  function releaseClaim(holdId) {
    db.prepare(`UPDATE voice_holds SET status = CASE WHEN expires_at > ? THEN 'active' ELSE 'expired' END
      WHERE id = ? AND status = 'confirming'`).run(new Date().toISOString(), holdId);
  }

  function findBookingByIdempotency(idempotencyKey) {
    const row = db.prepare(`SELECT b.*, s.name AS service_name, s.duration_minutes
      FROM voice_bookings b JOIN voice_services s ON s.id = b.service_id
      WHERE b.idempotency_key = ?`).get(idempotencyKey);
  ...
  function confirmHold({ hold, customerName, phone, email, providerUid, idempotencyKey }) {
    return transaction(() => {
      ...
      try {
        db.prepare(`INSERT INTO voice_bookings
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'confirmed', ?, ?, ?, ?)`)
          .run(bookingId, hold.tenantId, hold.serviceId, hold.id, hold.startAt, hold.endAt, customerName, phone, email || null, providerUid || null, idempotencyKey, now, now);
      } catch (cause) {
  ...
  function listBookings(currentTenant = tenantId, limit = 50) {
    return db.prepare(`SELECT b.*, s.name AS service_name, s.duration_minutes
      FROM voice_bookings b JOIN voice_services s ON s.id = b.service_id
      WHERE b.tenant_id = ? ORDER BY b.start_at DESC LIMIT ?`).all(currentTenant, limit).map(mapBooking);
  }

  function cancelBooking(bookingId, currentTenant = tenantId) {
    const booking = db.prepare(`SELECT b.*, s.name AS service_name, s.duration_minutes FROM voice_bookings b
      JOIN voice_services s ON s.id = b.service_id WHERE b.id = ? AND b.tenant_id = ?`).get(bookingId, currentTenant);
    if (!booking) return null;
    db.prepare(`UPDATE voice_bookings SET status = 'cancelled', updated_at = ? WHERE id = ?`).run(new Date().toISOString(), bookingId);
    return mapBooking({ ...booking, status: 'cancelled' });
  }

  function upsertCall(call) {
    const now = new Date().toISOString();
    const existing = call.externalId ? db.prepare('SELECT id FROM voice_calls WHERE external_id = ?').get(call.externalId) : null;
    const callId = existing?.id || call.id || id('CALL');
    db.prepare(`INSERT INTO voice_calls
      (id, external_id, tenant_id, provider, started_at, ended_at, duration_seconds, caller, intent, outcome, booking_id, transferred, cost, ai_disclosure, summary, ended_reason, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      .run(callId, call.externalId || callId, call.tenantId || tenantId, call.provider || 'vapi', call.startedAt || now, call.endedAt || null,
        call.durationSeconds || 0, call.caller || null, call.intent || null, call.outcome || 'IN_PROGRESS', call.bookingId || null,
        call.transferred ? 1 : 0, call.cost || 0, call.aiDisclosure === false ? 0 : 1, call.summary || null, call.endedReason || null, now, now);
    return callId;
  }

  function listCalls(currentTenant = tenantId, limit = 50) {
    return db.prepare(`SELECT id, external_id AS externalId, tenant_id AS tenantId, provider,
      ...
      FROM voice_calls WHERE tenant_id = ? ORDER BY started_at DESC LIMIT ?`).all(currentTenant, limit)
      .map((row) => ({ ...row, transferred: Boolean(row.transferred), aiDisclosure: Boolean(row.aiDisclosure), booking: row.summary || row.bookingId || null }));
  }
  ...
  function recordEvent({ tenantId: currentTenant = tenantId, callId = null, type, detail = null }) {
    ...
    db.prepare(`INSERT INTO voice_events (id, tenant_id, call_id, type, detail, created_at) VALUES (?, ?, ?, ?, ?, ?)`)
      .run(event.id, event.tenantId, event.callId, event.type, typeof detail === 'string' ? detail : JSON.stringify(detail || {}), event.createdAt);
    return event;
  }

  function stats(currentTenant = tenantId) {
    const calls = db.prepare(`SELECT COUNT(*) AS calls, COALESCE(SUM(cost),0) AS cost,
      ...
      FROM voice_calls WHERE tenant_id = ?`).get(currentTenant);
    const bookings = db.prepare(`SELECT COUNT(*) AS bookings FROM voice_bookings WHERE tenant_id = ? AND status = 'confirmed'`).get(currentTenant).bookings;
