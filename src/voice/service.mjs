...
function verifySlot(token, secret) {
  ...
  if (!encoded || !provided) throw error('Nieprawidłowy identyfikator terminu.', 'SLOT_TOKEN_INVALID', 400);
  const expected = crypto.createHmac('sha256', secret).update(encoded).digest();
  const actual = Buffer.from(provided, 'base64url');
  if (actual.length !== expected.length || !crypto.timingSafeEqual(actual, expected)) throw error('Podpis terminu jest nieprawidłowy.', 'SLOT_TOKEN_INVALID', 400);
  const payload = JSON.parse(unb64(encoded));
  if (payload.exp < Date.now()) throw error('Lista terminów wygasła. Sprawdź dostępność ponownie.', 'SLOT_TOKEN_EXPIRED', 409);
  return payload;
}
...
function toolArguments(toolCall) {
  const raw = toolCall.arguments ?? toolCall.parameters ?? toolCall.function?.arguments ?? toolCall.function?.parameters ?? {};
...
export function createVoiceService({ config, state, databasePath }) {
  ...
  async function availability({ service: serviceValue, preferredDate, timeRange = 'any' }) {
    ...
    const slots = calendarSlots
      ...
      .slice(0, 8)
      .map((slot) => ({
        id: signSlot({ tenantId, serviceId: service.id, startAt: slot.start, endAt: slot.end, exp: Date.now() + 15 * 60_000 }, voice.slotSecret),
  ...
  async function createHold({ slotId }) {
    ...
    try {
      ...
      if (providerReservationUid) await calendar.releaseReservation(providerReservationUid).catch(() => {});
      throw caught;
    }
  }
  ...
  function dashboard() {
    ...
    return {
      ...
      metrics: {
        ...
        bookings: Number(totals.bookings) || calls.filter((call) => call.outcome === 'BOOKED').length,
        ...
        toolSuccess: 100,
        aiDisclosure: totals.calls ? Math.round(Number(totals.disclosed || 0) / Number(totals.calls) * 100) : 100,
  ...
  async function handleToolCall(toolCall, callId) {
    ...
    if (name === 'confirm_booking') return confirmBooking({
      holdId: args.holdId,
  ...
  async function handleVapiMessage(payload) {
    ...
    if (message.type === 'end-of-call-report') {
      ...
      db.upsertCall({
        ...
        outcome: structured.outcome || (call.endedReason?.includes('transfer') ? 'TRANSFERRED' : 'UNRESOLVED'),
