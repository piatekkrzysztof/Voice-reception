...
export function validateConfig(config) {
  const issues = [];
  if (!Number.isInteger(config.port) || config.port < 1 || config.port > 65535) issues.push('PORT musi być liczbą od 1 do 65535.');
  if (!['sqlite', 'postgres'].includes(config.database.provider)) issues.push('Nieobsługiwany provider bazy danych.');
  if (config.database.provider === 'postgres' && !/^postgres(ql)?:\/\//.test(config.database.url)) issues.push('DATABASE_URL musi być prawidłowym adresem PostgreSQL.');
  if (!['disable', 'no-verify', 'verify-full'].includes(config.database.sslMode)) issues.push('DATABASE_SSL_MODE musi mieć wartość disable, no-verify albo verify-full.');

  if (config.production) {
    if (!/^https:\/\//.test(config.publicBaseUrl)) issues.push('PUBLIC_BASE_URL musi używać HTTPS w środowisku produkcyjnym.');
    if (config.database.provider !== 'postgres') issues.push('Środowisko produkcyjne wymaga DATABASE_URL do PostgreSQL.');
    if (config.voice.slotSecret.length < 32 || config.voice.slotSecret === 'local-development-slot-secret') issues.push('VOICE_SLOT_SECRET musi mieć co najmniej 32 losowe znaki.');
    if (config.auth.setupToken.length < 32) issues.push('VOICE_SETUP_TOKEN musi mieć co najmniej 32 losowe znaki.');
    if (!config.allowLocalProviders && (config.voice.provider === 'local' || config.voice.calendarProvider === 'local')) {
      issues.push('Produkcja wymaga Vapi i Cal.com. Dla kontrolowanego demo ustaw ALLOW_LOCAL_PROVIDERS=true.');
    }
    if (config.voice.provider === 'vapi') {
      if (!config.voice.webhookSecret) issues.push('VOICE_WEBHOOK_SECRET jest wymagany dla Vapi.');
      if (!config.voice.vapi.apiKey || !config.voice.vapi.assistantId || !config.voice.vapi.phoneNumberId || !config.voice.vapi.serverCredentialId) {
    ...
    if (config.voice.calendarProvider === 'calcom') {
      if (!config.voice.calcom.apiKey || Object.values(config.voice.calcom.eventTypes).some((value) => !value)) {
...
export function publicVoiceConfig(config) {
  ...
  return {
    ...
    integrations: {
      database: {
        name: 'Booking DB',
        detail: config.database.provider === 'postgres' ? 'PostgreSQL / pool' : 'SQLite / transakcje',
